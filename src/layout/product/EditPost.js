...
export const EditPost = ({route, navigation}) => {
  ...
  useEffect(() => {
    try {
      ...
      setname(tenu);
      setnoidung(noidungu);
      setlocation(checkinu);
      setnoidung1(hoatdongu);
      settick(ticku);
      setidp(idPostu);
      setidm(userIDu);
      settg(timeu);
  ...
  const AddPost = () => {
    ...
    if (
      ...
      if (isCheckedStatus == true) {
        const reference13 = firebaseDatabaseRef(db, 'post/' + user + '/' + idp);
        update(reference13, {
          name: name,
          avt: avt,
          id: idp,
      ...
      if (isCheckedStory == true && uplink != undefined) {
        const reference13 = firebaseDatabaseRef(
          db,
          'story/' + user + '/' + idp,
        );
        update(reference13, {
      ...
      ToastAndroid.show('Đã cập nhật bài viết', ToastAndroid.BOTTOM);
      navigation.goBack();
    }
  };
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>
                Bạn muốn cập nhật bài viết ở đâu!
              </Text>
